...
void Sprite::DrawScaled( int x1, int y1, int w, int h, Surface* target )
{
	...
	for (int x = 0; x < w; x++)
	{
		for (int y = 0; y < h; y++)
		{
			...
			// ChatGPT helped with including "currentFrame * width", which results in SetFrame() working in DrawText();
			uint color = GetBuffer()[(currentFrame * width) + u + v * width * numFrames];
