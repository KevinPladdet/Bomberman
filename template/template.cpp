...
int main()
{
	...
	{
		deltaTime = min( 100.0f, 1000.0f * timer.elapsed() );
		if (timer.elapsed() > 1.f / 60.f) // Cap FPS
		{
			timer.reset();
			app->Tick(deltaTime);
			...
			if (frameNr++ > 1)
			{
				...
				glfwPollEvents();
				int displayFPS = static_cast<int>(1000.f / deltaTime);
				glfwSetWindowTitle(window, ("Bomberman FPS: " + std::to_string(displayFPS)).c_str());
			}
			if (!running) break;
		}
	}
